...
class recognizer(object):
    ...
    def recFace(self,picture):
        ...
        img = cv2.resize(frame, (frame.shape[1]/1, frame.shape[0]/1), interpolation = cv2.INTER_CUBIC)
        imgout = img.copy()
        name='nesuno'
...
class faceRecognizer:
    ...
    def Init(self):
        ...
        parser.add_option("-c", "--cascade", action="store", dest="cascade_filename", default="haarcascade_frontalface_default.xml",
        ...
        self.__faceRecognizer=recognizer(model=model,
            camera_id=options.camera_id,
            cascade_filename=options.cascade_filename)
    def recFace(self,picture):
        name=self.__faceRecognizer.recFace(picture)
        return name
...
'''
    to run sample:  python simple_videofacerec.py -t 'faces' my_model.pkl
